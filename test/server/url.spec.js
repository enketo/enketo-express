...
describe('URL functionality', () => {
    describe('toLocalMediaUrl function', () => {
        ...
        it('should return proxied url', () => {
            expect(toLocalMediaUrl('http://foo.bar/fum/baz')).to.equal(
                'http://enke.to/media/get/http/foo.bar/fum/baz'
            );
        });

        it('escapes spaces in the path', () => {
            expect(toLocalMediaUrl('http://foo.bar/fum baz')).to.equal(
                'http://enke.to/media/get/http/foo.bar/fum%20baz'
            );
        });
    });
    ...
    describe('toMediaMap', () => {
        ...
        it('escapes spaces in mapped paths', () => {
            ...
            expect(toMediaMap(manifest)).to.deep.equal({
                'an%20image.jpg':
                    'http://enke.to/media/get/https/example.com/an%20image.jpg',
                'beastie%20boys.mp4':
                    'http://enke.to/media/get/https/example.com/beastie%20boys.mp4',
                'cd%20rip.mp3':
                    'http://enke.to/media/get/https/example.com/cd%20rip.mp3',
            });
        });
    });
});
