    ...
    describe( 'getting file URLs', () => {
        ...
        describe( 'instance attachments', () => {
            ...
            it( 'gets a URL from instance attachments by filename', async () => {
                ...
                expect( result ).to.equal( 'https://example.com/path/to/relative.png' );
            } );

            it( 'gets a URL from instance attachments by filename with a space', async () => {
                fileManager.setInstanceAttachments( {
                    'space madness.png': 'https://example.com/path/to/space%20madness.png',
                } );

                const result = await fileManager.getFileUrl( 'space madness.png' );

                expect( result ).to.equal( 'https://example.com/path/to/space%20madness.png' );
            } );

            it( 'gets a URL from instance attachments by filename with an escaped space', async () => {
                fileManager.setInstanceAttachments( {
                    'space%20madness.png': 'https://example.com/path/to/space%20madness.png',
                } );
            ...
            it( 'gets a URL from instance attachments by escaped filename with an escaped space', async () => {
                ...
                const result = await fileManager.getFileUrl( 'space%20madness.png' );
        ...
        describe( 'cached resources', () => {
            ...
            it( 'fails if the cache store is not available', async () => {
                ...
                try {
                    await fileManager.getFileUrl( 'https://example.com/anything.png' );
                } catch( error ) {
                    caught = error;
                }
