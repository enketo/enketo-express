...
echo 'copy redis-server configs'
...
if [ -f "/var/lib/redis/redis.rdb"]; 
then
	rm /var/lib/redis/redis.rdb
fi
echo 'copying default db'
...
echo 'starting first redis instance...'
service redis-server-enketo-main start
echo 'starting second redis instance'
